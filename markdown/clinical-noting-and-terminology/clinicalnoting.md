...
- [2 ADMISSIONS CLERKING GUIDANCE OVERVIEW](#2-admissions-clerking-guidance-overview)
  - [2.1 Rationale Summary](#21-rationale-summary)
  - [2.2 Summary of Guidance](#22-summary-of-guidance)
- [3 ENTERING A LIST OF NOTE ENTRIES](#3-entering-a-list-of-note-entries)
  - [3.1 Introduction](#31-introduction)
  - [3.2 Principles](#32-principles)
  - [3.3 Guidelines](#33-guidelines)
    - [3.3.1 Entering the Clinical Concept](#331-entering-the-clinical-concept)
    - [3.3.2 Dealing with ‘Match Not Found’](#332-dealing-with-match-not-found)
    - [3.3.3 Entering Additional Details](#333-entering-additional-details)
    - [3.3.4 Editing and Deleting Entries](#334-editing-and-deleting-entries)
- [4 REVEALING AND HIDING SECTIONS OF A SET OF DATA](#4-revealing-and-hiding-sections-of-a-set-of-data)
  - [4.1 Introduction](#41-introduction)
  - [4.2 Principles](#42-principles)
  - [4.3 Guidelines](#43-guidelines)
    - [4.3.1 Accessing Hidden Fields](#431-accessing-hidden-fields)
    - [4.3.2 Communicating That There Are Hidden Fields](#432-communicating-that-there-are-hidden-fields)
    - [4.3.3 Hiding Fields](#433-hiding-fields)
    - [4.3.4 Displaying Hierarchically Nested Fields](#434-displaying-hierarchically-nested-fields)
- [5 REQUIRED FIELDS](#5-required-fields)
  - [5.1 Introduction](#51-introduction)
  - [5.2 Principles](#52-principles)
  - [5.3 Guidelines](#53-guidelines)
    - [5.3.1 Proactively Indicating Required Fields](#531-proactively-indicating-required-fields)
    - [5.3.2 Reactively Indicating Required Fields That Have Been Missed](#532-reactively-indicating-required-fields-that-have-been-missed)
- [6 DISPLAYING PREVIOUS VALUES](#6-displaying-previous-values)
  - [6.1 Introduction](#61-introduction)
  - [6.2 Principles](#62-principles)
  - [6.3 Guidelines](#63-guidelines)
    - [6.3.1 Displaying Previous Values](#631-displaying-previous-values)
    - [6.3.2 Contextual Attributes to Display for Previous Values](#632-contextual-attributes-to-display-for-previous-values)
- [7 AUTOMATIC CALCULATIONS DATA](#7-automatic-calculations-data)
  - [7.1 Introduction](#71-introduction)
  - [7.2 Principles](#72-principles)
  - [7.3 Guidelines](#73-guidelines)
    - [7.3.1 Displaying Calculated Values](#731-displaying-calculated-values)
    - [7.3.2 Displaying Scores Where Values Are Missing](#732-displaying-scores-where-values-are-missing)
- [8 ADDING FREE TEXT](#8-adding-free-text)
  - [8.1 Introduction](#81-introduction)
  - [8.2 Principles](#82-principles)
  - [8.3 Guidelines](#83-guidelines)
    - [8.3.1 Add New Free Text](#831-add-new-free-text)
    - [8.3.2 Edit Free Text](#832-edit-free-text)
    - [8.3.3 Required or Recommended Free Text Field](#833-required-or-recommended-free-text-field)
- [9 GENERAL FORM DESIGNS](#9-general-form-designs)
  - [9.1 Introduction](#91-introduction)
  - [9.2 Principles](#92-principles)
  - [9.3 Guidelines](#93-guidelines)
    - [9.3.1 Use of Standard Fields](#931-use-of-standard-fields)
    - [9.3.2 Labels and Prompts](#932-labels-and-prompts)
- [10 DOCUMENT INFORMATION](#10-document-information)
  - [10.1 Terms and Abbreviations](#101-terms-and-abbreviations)
  - [10.2 Definitions](#102-definitions)
  - [10.3 Nomenclature](#103-nomenclature)
    - [10.3.1 Body Text](#1031-body-text)
    - [10.3.2 Cross References](#1032-cross-references)
...
- [APPENDIX B STUDY ID 34: EXECUTIVE SUMMARY](#appendix-b-study-id-34-executive-summary)
  ...
  - [B.4 Results](#b4-results)
    ...
    - [B.4.2 Key Findings](#b42-key-findings)
    - [B.4.3 Conclusions](#b43-conclusions)
  - [B.5 Design Iterations: Findings and Design Updates](#b5-design-iterations-findings-and-design-updates)
...
Source PDF: [clinicalnoting.pdf](../../pdfs/clinical-noting-and-terminology/clinicalnoting.pdf)
...
In the context of the data that needs to be captured as part of Admissions Clerking (as defined in the RCP Admissions Clerking Standard, _Hospital Admission Pro-forma Headings and Definitions_ **{R2}** ), the guidance produced in this document will address the following Clinical Noting areas:
...
**A4** The RCP standards addressing record keeping in acute admissions will be applied to the creation of a pro-forma ( _A_ _Clinician’s Guide to Record Standards – Part 2_ **{R1}** and _Hospital Admission Pro-forma Headings and Definitions_ **{R2}** ).
...
_Ergonomic requirements for office work with visual display terminals (VDTs) -- Part 10:_ _Dialogues principles_ **{R9}** )
...
Finally, this guidance only addresses the action of entering data and the display of data _during its_ _entry_. It does not cover how the data should be displayed at a later time.
...
The mechanism for SNOMED CT matching, including the entry field size and the visual display used for distinguishing common matches with other matches are based upon previous CUI guidance. For example, the width of the search entry field (32 characters) was determined in _Design Guide Entry – Terminology – Matching_ **{R14}**, based upon analysis of the lengths of SNOMED CT terms. Additionally, the display of the encoded term is also drawn from previous CUI guidance (see _Design Guide Entry – Terminology –_ _Display Standards for Coded Information_ **{R16}** ).

The use of ‘common’ items is sometimes used in existing paper forms in order to provide quicker entry and prompting (for example, in _Hospital Admission Pro-forma Headings and Definitions_ **{R2}** ). This feature is also supported by an international standard, which states that if the “frequency of option use is known (or can be determined) and option groups are small (eight or less), the most frequently used options should be placed first” in a menu (see _Menu dialogues_ **{R6}** ).

In a recent paper outlining principles for effective clinical decision support, the authors referenced research that suggested clinicians’ satisfaction with a clinical system markedly declines if the time taken to provide system feedback exceeds one second. The authors therefore suggested ‘subsecond screen flips’ (see _Ten Commandments for Effective Clinical Decision Support: Making the Practice of_ _Evidence-based Medicine a Reality_ **{R5}** ). This supports the need for an animation to appear if the screen change exceeds one second **{R5}** . The need for “timely and perceptible feedback” associated with “normal task performance” which is “non-intrusive” and does not “distract the user from the task” is also supported by international standards (see _User guidance_ **{R10}** ). Another standard indicates that “if the system response to option execution” is delayed, “an indication should be provided to the user that the system is processing the request” **{R6}** . This standard defines a delay as being “more than 3 s[econds] after initiation” **{R6}**, although the current guidance authors argue that the need to reduce frustration on the part of the clinician would point to providing the indication quicker than 3 seconds.
...
The need for a visible cursor is also underlined by a relevant standard, which states that “cursor position should always be clearly visible if it is within the currently displayed portion of the form” (see _Form-filling dialogues_ **{R7}** ).
...
Locating the duration and/or date field _**before**_ the additional text field is suggested in order to encourage clinicians _**not**_ to enter duration and/or date information as free text.
...
Relevant ISO guidance states that ‘all fields should be clearly and unambiguously labelled to describe what kind of content should be entered’ (see _Form-filling dialogues_ **{R7}** ) and that these labels must be ‘distinctive’ in relation to data or instructions, for example, using visual cues such as position, font or colour, and that these visual cues should be applied consistently throughout the form. Therefore, provide sufficient labelling of the fields. These labels can be in-field ‘prompts’, as defined in the _Microsoft Windows User_ _Experience Interaction Guidelines_ **{R22}** .

Previous CUI guidance on terminology matching and elaboration demonstrates the need for an additional text field to complement the single concept matching field. It also demonstrates how qualifier concepts, such as severity, can be matched within the additional text field (See _Design Guide Entry – Date and Time Input_ **{R13}** and _Design Guide Entry – Terminology – Matching_ **{R14}** ).

Style guidance for established UIs indicates that the free text field must be a minimum of two lines high and that a vertical scroll bar appears after the clinician’s typed entry exceeds the visible space (see _Essential guide to user interface design_ **{R21}** ).
...
This section addresses how this should be done in a basic manner, which conforms to traditional electronic form design (for details of traditional electronic form designs, see _Essential guide to user_ _interface design_ **{R21}** ).
...
**R6.** British Standards Institute, BS EN ISO 9241-14: 1997 Ergonomic requirements for office work with visual display terminals (VDTs) _--_ Part 14: Menu dialogues
...
See _Designing_ _the User Interface – Strategies for Effective Human-Computer Interaction_ **{R20}** for more information on these principles:
...
See _Ergonomic requirements for office work with visual display terminals (VDTs) -- Part 10:_ _Dialogues principles_ **{R9}** for more information on these principles:
