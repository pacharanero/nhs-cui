...
- [2 RECOMMENDATION AND GUIDANCE](#2-recommendation-and-guidance)
  - [2.1 Approaches to Matching SNOMED-CT Concepts to Clinicians’ Notes](#21-approaches-to-matching-snomed-ct-concepts-to-clinicians-notes)
    - [2.1.1 Single Concept Approach to Matching](#211-single-concept-approach-to-matching)
  - [ Offer the single concept matching n/a Although the Single concept matching approach](#offer-the-single-concept-matching-na-although-the-single-concept-matching-approach)
    - [2.1.2 Text Parser Approach to Matching](#212-text-parser-approach-to-matching)
  - [2.2 Inputting Notes](#22-inputting-notes)
    - [2.2.1 Input in the Text Parser Approach](#221-input-in-the-text-parser-approach)
    - [2.2.2 Input in the Single Concept Approach](#222-input-in-the-single-concept-approach)
    - [2.2.3 Additional Text Input](#223-additional-text-input)
    - [2.2.4 Spell-Checker Input](#224-spell-checker-input)
    - [2.2.5 Dealing with Clinicians’ Shorthand](#225-dealing-with-clinicians-shorthand)
  - [2.3 Matching](#23-matching)
    - [2.3.1 Matching Input Text Against the SNOMED-CT Database](#231-matching-input-text-against-the-snomed-ct-database)
  - [ Switching on word equivalence](#switching-on-word-equivalence)
    - [2.3.2 Presenting SNOMED-CT Matches](#232-presenting-snomed-ct-matches)
  - [ Feature numbering next to each list item ‘1. Anxiety In testing, users were confused by a system of](#feature-numbering-next-to-each-list-item-1-anxiety-in-testing-users-were-confused-by-a-system-of)
    - [2.3.3 Selecting and Confirming SNOMED-CT Matches](#233-selecting-and-confirming-snomed-ct-matches)
    - [2.3.4 Dealing with Abbreviations](#234-dealing-with-abbreviations)
  - [ Ensure that abbreviations are always](#ensure-that-abbreviations-are-always)
    - [2.3.5 ‘Undoing’ Matches](#235-undoing-matches)
  - [2.4 Context](#24-context)
    - [2.4.1 Optimising Matching by the Use of Contextual Limitation](#241-optimising-matching-by-the-use-of-contextual-limitation)
  - [2.5 Refinement](#25-refinement)
    - [2.5.1 Refining a Selected Concept](#251-refining-a-selected-concept)
...
Source PDF: [termmatching.pdf](../../pdfs/clinical-noting-and-terminology/termmatching.pdf)
...
The document _Table of Contents_ **{R1}** outlines every specific area of focus to be covered by the Design Guide. Each area of focus has an accompanying Design Guide Entry document. Table 1 (below) indicates the areas of focus covered by this Design Guide Entry.
...
The CUI team has shown how this approach to matching SNOMED-CT concepts may contribute to the clinical noting process. Further details are contained in the _Clinical Noting User Interface Vision_ _and Scope_ document **{R9}** .
...
This text will also be parsed for relevant attributes of the main concept (see _Design Guide Entry –_ _Terminology - Elaboration_ **{R4}** for details).
...
outcome of the post-coordination parsing process (see _DG-R4-003 Design Guide Entry –_ _Post-coordination_ **{R2}** )
...
The topic of possible ways to communicate concept definitions is further discussed in SNOMED _Clinical Terms ® Guide – Abstract logical models and Representational forms_ **{R12}.**
...
For further discussion about abbreviations and acronyms, please refer to _NHS CUI Design Guide_ _Workstream – Abbreviations and Acronyms_ **{R10}** and _NHS Design Guide and Toolkit Workstream_
...
- Must allow multiple subsets to apply simultaneously, if appropriate _**Low**_
...
Most users also felt that it is preferable to hide the navigation dialog unless it is specifically requested, as is the case in the current design. They felt that making this dialog visible too soon is overwhelming. As one user commented, ‘initially it needs to be simple’. Also, presenting the users with the navigation dialog without the user’s request, leads to users incorrectly thinking that they _**must**_ refine the concept.
